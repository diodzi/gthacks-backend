...
function computeWin(side, finalValue, betLine) {
    return side === 'over' ? finalValue > betLine : finalValue < betLine;
}
function computePayout(amount, oddsMultiplier) {
    return toInt(amount * oddsMultiplier);
}
export async function placeSingleBet(c) {
    const betId = Number(c.req.param('betId'));
    const body = await c.req.json();
    if (Number.isNaN(betId))
        return c.json({ error: 'Invalid betId' }, 400);
...
export async function settleSingleBet(c) {
    ...
    try {
        await db.transaction(async (tx) => {
            ...
            for (const t of tickets) {
                const didWin = computeWin(t.side, finalValueNum, betLine);
                if (didWin) {
